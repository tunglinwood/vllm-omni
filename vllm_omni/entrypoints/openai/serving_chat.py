...
class OmniOpenAIServingChat(OpenAIServingChat, AudioMixin):
    ...
    def _create_audio_choice(
        ...
        # Read actual sample rate from output (code2wav stage may resample)
        sr_tensor = mm_output.get("sr")
        if sr_tensor is not None:
            sample_rate = int(sr_tensor.item()) if hasattr(sr_tensor, "item") else int(sr_tensor)
        else:
            sample_rate = 24000
