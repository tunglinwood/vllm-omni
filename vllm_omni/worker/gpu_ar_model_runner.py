            ...
            for k, v in multimodal_outputs.items():
                try:
                    if isinstance(v, torch.Tensor) and v.shape[0] == hidden_states_cpu.shape[0]:
                        ...
                        # 1D tensors like audio_codes that don't match hidden_states
                        # batch dimension are per-request payloads. Pass through to
                        # every request (they get merged later by the output processor).
                        mm_cpu[k] = v.detach().to("cpu").contiguous()
                except Exception as e:
            ...
            if mm_cpu:
                ...
                for k, v in mm_cpu.items():
                    if isinstance(v, torch.Tensor) and v.shape[0] == hidden_states_cpu.shape[0]:
                        ...
                        # 1D or request-invariant tensor payloads; clone to
